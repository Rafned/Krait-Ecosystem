...
- **Хранилище:** `sled` с отдельными деревьями для DNA (`dna_tree`), индексов (`index_tree`).
- **Сжатие:** `zstd` level 3.
...
Krait — это гибридная мультиагентная система, в которой Rust-ядро берёт на себя инфраструктурную надёжность (коммуникации, персистентность, безопасность, планирование), а Python-слой — когнитивную логику (архитектурный дизайн, генерация кода, валидация). Ключевой инженерный акцент сделан на управление ограниченными GPU-ресурсами (VRAM-эвикция, сериализация LLM-узлов), гарантию синтаксической валидности выхода (детерминированный repair + многоуровневая валидация) и контролируемую оркестрацию сложных workflow (DAG с зависимостями и fallback-цепочками). Гибридная реализация MetaOvermind позволяет сочетать гарантированную доставку и таймауты Rust с когнитивной гибкостью Python/LLM. Три уровня агентов (L0-исполнители, L2-специалисты, L3-стратеги) образуют полный конвейер: от цели пользователя до синтаксически валидного многомодульного проекта с перекрёстными импортами и проверенной архитектурой.

---

### 12.  Статус реализации квантовых компонентов
**Касаемо квантовых вычислений, чтобы не было недопонимания: это в первую очередь симуляции. В архитектуре заложена работа с QPU в будущем.**

---

### 🟡 Работает (симуляция / MVP)
- `QuantumOracle` — VQE/QAOA на симуляторе PennyLane (GPU/CPU)
- `BizQuantumIntegrator` — LLM-перевод бизнес-требований в техрешения, интеграция с KG, комплаенс-заглушки

### 🔴 Заглушки / требует доработки
- `QuantumOracle`: Grover (ограничен 10 кубитами), адаптеры IBM/IonQ/Rigetti (ждут токены)
- `BizQuantumIntegrator`: квантовый Monte Carlo (использует симулятор), quantum speedup (эвристика)
- `GDPRChecker` / `PCIChecker` — API готов, бизнес-логика не реализована

###  Примечание
Квантовые алгоритмы работают **только в режиме симуляции** на CPU/GPU. 
Реальная интеграция с QPU (IBM, IonQ, Rigetti) запланирована когда-нибудь, но требует:
1. Получения API-токенов
2. Тестирования на реальном железе
3. Адаптации под шум реальных кубитов

Текущая реализация пригодна для:
...
*Проект находится в стадии активной разработки. Статус компонентов обновляется по мере реализации.*
